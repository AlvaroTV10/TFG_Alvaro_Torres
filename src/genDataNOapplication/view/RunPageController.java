...
public class RunPageController {
	...
	public int handleParametersButton() {
		int numP, numT, maxT, engT;
		double a, b, m, type, quality, vid, ima, tex,vid_w, ima_w, tex_w, mus, dri, tvs, spo, ani, com, neww, foo, tra, fas, car, jok, tech, wat, jew, dan, fit, arc, inter, cult, pen2, pen3, com_p, share_p, min_s;
		// Check input is parseable
				try {
					...
					engT = Integer.parseInt(engThres.getText());
			        ...
			        tra = Double.parseDouble(travel.getText());
			        fas = Double.parseDouble(fashion.getText());
			        car = Double.parseDouble(cars.getText());
			        jok = Double.parseDouble(jokes.getText());
			        tech = Double.parseDouble(technology.getText());
			        wat = Double.parseDouble(watches.getText());
			        jew = Double.parseDouble(jewellery.getText());
			        dan = Double.parseDouble(dance.getText());
			        fit = Double.parseDouble(fitness.getText());
			        arc = Double.parseDouble(architecture.getText());
			        inter = Double.parseDouble(interior.getText());
			        cult = Double.parseDouble(culture.getText());
				...
				if(engT <1) conditonsAlertText += "Engagement Threshold cannot be negative or 0" + "\n";
				
				this.configuration.setNumPublications(numP);
				this.configuration.setNumTime(numT);
				this.configuration.setMaxTagged(maxT);
				this.configuration.setEngagementThreshold(engT);
				...
				if(Math.abs(mus+ dri+ tvs+ spo+ ani+ com+ neww+ foo+ tra + fas+ car+ jok+ tech+ wat+ jew+ dan+ fit+ arc+ inter + cult -1) > 0.1)	conditonsAlertText += "Topics of publications sum must be 1" + "\n";
				else {
					List<Double> topics = Arrays.asList(mus, dri, tvs, spo, ani, com, neww, foo, tvs, fas, car, jok, tech, wat, jew, dan, fit, arc, inter, cult);	
